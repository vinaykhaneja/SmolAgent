...
async def draw_rectangle(point1: int, point2: int, point3: int, point4: int) -> dict:
    ...
    try:
        ...
        paint_window.click_input(coords=(937, 160))
        time.sleep(0.25)
...
async def add_text_in_paint(text: str, x1: int, y1: int) -> dict:
    ...
    try:
        ...
        canvas.click_input(coords=(350, 800))
...
async def open_paint() -> dict:
    ...
    try:
        ...
        win32gui.SetWindowPos(
            paint_window.handle,
            win32con.HWND_TOP,
            0, 0,  # Position it on first monitor
...
def send_email(to, subject, message_text):
    ...
    try:
        ...
        return (f"Email sent successfully with Message Id: {sent_message['id']}")
    except Exception as error:
        return (f"Email failed to send. An error occurred: {error}")
